...
?>
    ...
    <p>These boring, chunky polygons wouldn't fly in the polished, pretty portfolio I was planning to make. I wanted my letters to have nothing less than the smooth, sharp edges of vector shapes, so I devised a function that would run every animation frame. The function would take the positions and rotations of the physics objects, and translate this data into offsets and rotation values for absolutely positioned SVG images. This allowed me to use the clean SVG images as façades for my boring polygon physics bodies.</p>
    ...
    <p>The minimalist design looked great, but I didn't feel like it was doing enough to show off my design prowess. In researching other design portfolios, I found the portfolio of <a href="http://beta.rallyinteractive.com/">Rally Interactive</a>, a development firm whose website represented exactly what I wanted. In addition to the interactive and dynamic graphics, they also had header compositions for each project that conveyed the idea in an attractive graphic. I emulated this with what I called "tableaus": graphics at the top of each section that provided an attractive preview of the section's content.</p>
    
    <p>When I began composing the sections for each portfolio piece, I quickly discovered that it was highly tedious to re-export graphics for optimal file size. This drove me to completely redesign the section system and create a simple CMS to handle image processing.</p>
...
photogrid([section name], [folder name], [rows], [columns], [background position]);
    ...
    <p>I taught myself to use PHP's "GD" library, which processes images, in order to generate perfectly-sized jpegs that would load rapidly for visitors. I integrated this functionality into two PHP functions that I could use in any section. I designed the functions to be fool-proof; when the function is called, it checks to see if the correctly sized images already exist, and if not, then it resizes them and saves them for the next time the function is called. This way, no matter what I change the sizing of an image to, the function creates a version with the perfect size automatically.</p>
